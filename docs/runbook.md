...
**The bounds that stop saturation becoming a hang** (`apps/api/src/prisma/database-pool.ts`). Ten pooled connections; a request waits at most 5 s for one and then fails rather than queueing invisibly. Any single statement is cancelled at 10 s (`DATABASE_STATEMENT_TIMEOUT_MS`), which surfaces as Postgres `57014`; a transaction Prisma has stopped driving is dropped by Postgres at 15 s. Those numbers are a ladder, not three independent settings — statement < Prisma transaction < `idle_in_transaction`, so the fastest layer always wins and you get an error you can read instead of a dropped connection. `database-pool.spec.ts` asserts that ordering. `lock_timeout` is deliberately unset: it would turn seat contention into failures, and a statement blocked on a lock is already bounded by `statement_timeout`. Connections identify themselves as `openseat-api` in `pg_stat_activity`.
