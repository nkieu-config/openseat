...
- **The whole product runs in CI** — five jobs on every push: one gate that lints, typechecks, builds and unit-tests every workspace including both Go suites under `-race`, then 99 API integration tests against real Postgres and Redis, 10 browser journeys with a four-page accessibility scan, a dependency audit, and a smoke test that boots the built API image and probes it. Both published contracts are drift-gated — regenerate the OpenAPI schema or the GraphQL schema and CI fails if either moved.
